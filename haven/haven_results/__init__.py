...
class ResultManager:
    ...
    def get_plot_all(self, y_metric_list, order='groups_by_metrics', 
                     ...
                     savedir_plots=None, legend_last_row_only=False, show_legend_all=None, plot_median=False, plot_vs_stepsize=False,
                     **kwargs):
        """[summary]
            ...
            for j, exp_list in enumerate(exp_groups):   
                ...
                for i, y_metric in enumerate(y_metric_list):
                    ...
                    fig, _ = get_plot(exp_list=exp_list, savedir_base=self.savedir_base, y_metric=y_metric, 
                                    ...
                                    ylim=ylim, xlim=xlim,
                                    score_list_name=self.score_list_name, plot_median=plot_median, plot_vs_stepsize=plot_vs_stepsize,
                                    **kwargs)
                fig_list += [fig]
...
def get_result_dict(exp_dict, 
    ...
    #print(exp_dict)
    #if ("adaptive_termination" in exp_dict["opt"].keys() and exp_dict["opt"]["adaptive_termination"]) or exp_dict["opt"]["name"] in ["sls", "sls_svrg_ada", "sgd_ada_svrg_ada", "sgd_ada", "svrg_loopless"]:
    if True:
        if x_metric in ["n_grad_evals", "n_grad_evals_normalized"]:
            res1 = get_result_dict(exp_dict,
                                  savedir_base,
                                  "epoch",
                                  x_metric,
                                  exp_list=exp_list,
            ...
            res2 = get_result_dict(exp_dict,
                                  ...
                                  result_step=result_step)
            y_list = res2['y_list']     
            return {'y_list':res2['y_list'], 
                    'x_list':res1['y_list'],
                    'y_std_list':res2['y_std_list'],
                    'visited_exp_ids':res2['visited_exp_ids']}
        ...
        if len(x_dict) == 0:
            ...
            if y_list_all.dtype == 'object' or len(y_list_all)==0:
                ...
                if plot_confidence:
                    ...
                    y_std_list = 0
                if plot_median:
                    y_list = np.median(y_list_all, axis=1)
                else:
    ...
    if plot_vs_stepsize:
        ...
        for exp_dict, style_dict in zip(exp_list, style_list):
            result_dict = get_result_dict(exp_dict, 
                            ...
                            score_list_name=score_list_name,
                            result_step=result_step)
           # print(exp_dict['opt']['name'], exp_dict['opt']['init_step_size'])
            #print(result_dict['y_list'])
            
            exp_dict_no_run = copy.deepcopy(exp_dict)
            del exp_dict_no_run['runs']

            opt_name = exp_dict_no_run['opt']['name']
            if opt_name not in ["svrg_ada"]:
                if exp_dict['opt']['init_step_size'] not in x_list:
                    x_list.append(exp_dict['opt']['init_step_size'])
            y_end = result_dict['y_list'][-1]
            
            opt_init_step_size = exp_dict['opt']['init_step_size']
            #exp_dict_no_run_to_add = copy.deepcopy(exp_dict_no_run)
            del exp_dict_no_run["opt"]["init_step_size"]
            if str(exp_dict_no_run) not in opt_dict.keys():
                opt_dict[str(exp_dict_no_run)] = style_dict
            if (str(exp_dict_no_run), opt_init_step_size) in result_dict_test.keys():
                result_dict_test[(str(exp_dict_no_run), opt_init_step_size)].append(y_end)
            else:
                result_dict_test[(str(exp_dict_no_run), opt_init_step_size)] = [y_end]
                
                

        
        for key in result_dict_test.keys():
            if np.any(np.isnan(result_dict_test[key])) or np.any(np.isinf(result_dict_test[key])):
                default_value = 10
                result_dict_test[key] = default_value
            else:
                if plot_median:
                    result_dict_test[key] = np.median(result_dict_test[key])
                else:
                    result_dict_test[key] = np.mean(result_dict_test[key])
                  
                default_value = 10
                if result_dict_test[key] > default_value:
                    result_dict_test[key] = default_value

        x_list.sort()
        for exp_dict_no_run in opt_dict.keys():
            
            y_list = []
            #exp_dict_no_run = ast.literal_eval(exp_dict_no_run_to_add)
            if ast.literal_eval(exp_dict_no_run)["opt"]["name"] in ["svrg_ada"]:
                for x_value in x_list:
                    y_list.append(result_dict_test[(exp_dict_no_run, 1)])
            else:
                for x_value in x_list:
                    #exp_dict_no_run["opt"]["init_step_size"] = x_value
                    y_list.append(result_dict_test[(exp_dict_no_run, x_value)])
            style_dict = opt_dict[exp_dict_no_run]
            #print(opt_name, y_list)
