...
#SGD with exponential decreasing step size
opt_list += [{'name': 'EXP_SGD',
           'r': 1,
           'alpha_t': "DECR",
            'is_sls': False,
...
#SGD with exponential decreasing step size and SLS
opt_list += [{'name': 'EXP_SGD',
               'r': 1,
               'alpha_t': "DECR",
                'is_sls': True,
...
#ASGD with constant and decreasing step size
for alphat in ["CNST", "DECR"]:
...
for benchmark in benchmarks_list:
        EXP_GROUPS['exp_%s' % benchmark] += hu.cartesian_exp_group(get_benchmark(benchmark, opt_list,
                                                                                 batch_size=1, max_epoch=[MAX_EPOCH], runs=RUNS,losses=['squared_loss','logistic_loss']))
